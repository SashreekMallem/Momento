export class AnalyticsService {
    ...
    async analyzeUserBehavior(userId, timeRange = 'month') {
    ...
    analyzeEngagement(events, missions, timeRange) {
    ...
    analyzeBehavioralPatterns(patterns, missions) {
    ...
    analyzePersonalization(profile, patterns) {
    ...
    calculateProfileCompleteness(profile) {
        ...
        if (profile.privacy_preferences)
            completeness += 0.1;
        if (profile.app_preferences)
            completeness += 0.1;
        if (profile.onboarding_completed)
            completeness += 0.1;
        return Math.min(completeness, 1.0);
    }
    generateInsights(engagement, behavioral, personalization) {
        const strengths = [];
        const opportunities = [];
    ...
    async generatePersonalizationReport(userId) {
    ...
    calculateOverallScore(analytics) {
        const engagementScore = (analytics.engagement.missionCompletionRate * 0.3 +
            Math.min(analytics.engagement.averageSessionDuration / 600, 1) * 0.2 // Cap at 10 minutes
        );
        const behavioralScore = (analytics.behavioral.consistencyScore * 0.2 +
            (analytics.behavioral.growthTrajectory === 'improving' ? 0.1 :
                analytics.behavioral.growthTrajectory === 'stable' ? 0.05 : 0));
        const personalizationScore = (analytics.personalization.profileCompleteness * 0.1 +
