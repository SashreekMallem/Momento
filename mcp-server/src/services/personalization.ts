...
export class PersonalizationEngine {
  ...
  async getCompleteUserProfile(userId: string): Promise<CompleteUserProfile> {
    const [profile, people, tastes, behavioralPatterns, missionStats] = 
      await Promise.all([
  ...
  private inferPersonalityTraits(profile: any, tastes: any, behavioralPatterns: any[]): string[] {
    const traits: string[] = [];

    // Analyze life themes
    const lifeThemes = profile?.life_themes?.selectedThemes || [];
    if (lifeThemes.includes('adventure')) traits.push('adventurous');
    if (lifeThemes.includes('relationships')) traits.push('social');
    if (lifeThemes.includes('growth')) traits.push('growth-oriented');
    if (lifeThemes.includes('reflection')) traits.push('introspective');
    if (lifeThemes.includes('wellness')) traits.push('health-conscious');

    // Analyze music preferences
    if (tastes.music?.genres?.includes('classical')) traits.push('sophisticated');
    if (tastes.music?.genres?.includes('jazz')) traits.push('cultured');
    if (tastes.music?.genres?.includes('rock')) traits.push('energetic');
    if (tastes.music?.music_moods?.includes('Focus & Study')) traits.push('focused');

    // Analyze food preferences
    if (tastes.food?.cuisines?.length > 5) traits.push('culinary-adventurous');
    if (tastes.food?.dietary_restrictions?.length > 0) traits.push('health-conscious');
    if (tastes.food?.cooking_interests?.length > 0) traits.push('creative');
  ...
  private inferPreferredActivities(tastes: any, missionStats: any): string[] {
    const activities: string[] = [];

    // From taste preferences
    if (tastes.music?.favorite_artists?.length > 0) activities.push('music-listening');
    if (tastes.music?.music_moods?.includes('💪 Energetic & Pumped')) activities.push('exercise');
    if (tastes.food?.cooking_interests?.length > 0) activities.push('cooking');
    if (tastes.movie?.genres?.includes('documentary')) activities.push('learning');
  ...
  private inferSocialPreferences(profile: any, people: any[]): string {
    const coupleHub = profile?.life_themes?.coupleHub;
    const familyMode = profile?.life_themes?.familyMode;
    const peopleCount = people.length;

    if (coupleHub && familyMode) return 'family-oriented';
    if (coupleHub) return 'couple-focused';
    if (peopleCount > 10) return 'highly-social';
    if (peopleCount > 5) return 'social';
    if (peopleCount > 0) return 'selective-social';
    return 'independent';
  }

  private inferTimePreferences(profile: any, behavioralPatterns: any[]): string[] {
    const preferences: string[] = [];

    const timeCommitment = profile?.life_themes?.timeCommitment;
    if (timeCommitment === 'light') preferences.push('short-sessions');
    if (timeCommitment === 'moderate') preferences.push('medium-sessions');
    if (timeCommitment === 'deep') preferences.push('long-sessions');
    ...
    behavioralPatterns.forEach(pattern => {
      if (pattern.pattern_type === 'peak_activity_time') {
        ...
        if (timeData.morning_activity > 0.7) preferences.push('morning-person');
        if (timeData.evening_activity > 0.7) preferences.push('evening-person');
      }
    });
  ...
  private analyzeEngagementPatterns(missionStats: any, behavioralPatterns: any[]) {
    return {
  ...
  private calculateConsistencyScore(behavioralPatterns: any[]): number {
  ...
  private identifyMotivationFactors(behavioralPatterns: any[]): string[] {
  ...
  private async calculatePersonalizationScore(userId: string): Promise<number> {
    ...
    if (profile?.life_themes) score += 0.1;
    if (profile?.onboarding_completed) score += 0.1;
    if (profile?.location) score += 0.05;
    if (profile?.timezone) score += 0.05;
    if (profile?.notification_preferences) score += 0.1;

    // Relationship data (25%)
    if (people.length > 0) score += 0.1;
    if (people.length > 3) score += 0.1;
    if (people.some((p: any) => p.person_interests?.length > 0)) score += 0.05;

    // Taste preferences (25%)
    if (tastes.music?.genres?.length > 0) score += 0.08;
    if (tastes.food?.cuisines?.length > 0) score += 0.08;
    if (tastes.movie?.genres?.length > 0) score += 0.09;

    // Activity and engagement (10%)
    if (events.length > 10) score += 0.05;
    if (events.length > 50) score += 0.05;

    return Math.min(score, 1.0); // Cap at 1.0
  }

  private async updateBehavioralPatternsFromPreferences(
    userId: string,
    preferences: any
  ): Promise<void> {
    ...
    if (preferences.theme) {
      await this.databaseService.updateBehavioralPattern(
        userId,
        'ui_preferences',
        {
  ...
  async adaptUserExperience(userId: string): Promise<any> {
  ...
  private getRecommendedMissionTypes(profile: CompleteUserProfile): string[] {
    const types: string[] = [];
  ...
  private getOptimalScheduling(profile: CompleteUserProfile): any {
    return {
  ...
  private getPersonalizedUISettings(profile: CompleteUserProfile): any {
    return {
  ...
  private getPreferredCommunicationStyle(profile: CompleteUserProfile): string {
  ...
  private calculateOptimalFrequency(profile: CompleteUserProfile): string {
    ...
    if (engagement > 0.8 && consistency > 0.7) return 'daily';
    if (engagement > 0.6 && consistency > 0.5) return 'every-other-day';
    if (engagement > 0.4) return 'weekly';
    return 'bi-weekly';
  }
}
